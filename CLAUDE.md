...
**Decided 2026-09-19: adopt AnymeX's extension bridge for its *Aniyomi*
backend, and keep `lib/source/` as the Mangayomi one.** The first version of
this decision was "stop maintaining a parallel source runtime"; measuring the
two against each other reversed that half. See "The extension bridge" below.
...
that — four attempts, all recorded in `tool/`.

The conflict is the **generator fork**, not `d4rt`. Move `isar_community` to
`3.3.0-dev.3` with the *published* generator of the same version, and the
analyzer-8 requirement leaves with the fork:

| | this repo | the bridge's |
|---|---|---|
| `d4rt` | `^0.2.4` | `0.1.7` |
| Isar | `^3.3.2` + Mangayomi's generator fork (analyzer 8) | `3.3.0-dev.3` + the published generator (analyzer 7) |

### …and `lib/source/` does not have to go — measured

The first version of this section said the resolution was to **drop
`lib/source/` and take the bridge's runtime**, because the conflict was this
app's own `d4rt ^0.2.4`. That was reasoning from the pin, not from the code.
Measured instead:

**This app's runtime runs on `d4rt 0.1.7` with one line changed.** `0.2.4`
deprecated `args: Object?` in favour of `positionalArgs: List<Object?>`; `0.1.7`
has only `args`, and wraps a single object into `[args]` itself, so
`positionalArgs: [source.toMSource()]` becomes `args: source.toMSource()` and
means the same thing. Nothing else in the 17 files that import `d4rt` needed
touching — the `BridgedClass` surface is unchanged between the two versions,
which is checkable directly: the bridge's own `Eval/dart/bridge/*.dart` and
ours are the same shape, because both are ports of the same upstream.

On `d4rt 0.1.7` + Isar `3.3.0-dev.3` + the published generator:

- `flutter analyze` clean; `build_runner` output identical but for the
  `version:` string it stamps into each schema;
- **512 tests pass**;
- **the live sweep holds the baseline exactly** — evaluated 55/55, popular 8,
  detail+chapters 6, full chain 6. The same six sites, the same chapter and
  page counts.

And with those pins the bridge resolves **alongside** `lib/source/` — added as
a git dependency, `flutter pub get` succeeds with no overrides at all, and
`flutter analyze` stays clean.

So the bridge is **additive**. There is no forced deletion, nothing to
migrate, and the door stays open without paying anything today — which is why
the pins in the toolchain table above are **not** being moved yet. A downgrade
to `d4rt 0.1.7` buys nothing until the bridge actually lands, and the Isar
move contradicts a rule recorded there. What has been bought is the knowledge
that it costs one line when the time comes.

### The bridge's Mangayomi path is lossier than this app's — also measured

Diffed `MangayomiSourceMethods` against `DartSourceRuntime`, which is the
comparison that decides whether slice 1 is an upgrade:

| | the bridge | here |
|---|---|---|
| publication status | **`DMedia` has no `status` field at all.** The extension sets `MManga.status`; the mapper reads the object and drops it | `MManga.status`, rendered on the details screen |
| detail correcting the title/cover | no — `getDetail` returns `media.title` and `media.cover` **from the object passed in**, so `getDetail(DMedia.withUrl(url))` comes back with an empty title and an empty cover | the extension's own values |
| a failed `getPageList` | `catch { return [] }` — a Cloudflare block and a chapter with no pages are the same answer | propagates |
| `cancelRequest` | `throw UnimplementedError()` | — |
| interpreter lifetime | **a new `D4rt` per call**: every method runs `_executeLib()`, which constructs an interpreter, registers the bridge and re-evaluates the whole script | one per source, cached and fingerprinted over script + `baseUrl` + `apiUrl` + `additionalParams` |
| browse rows | `_mapMediaList` keeps title/url/cover/description/**artist** and drops `author` and `genre` | carried |

None of that is a defect in the bridge. `DMedia` is one model for seven
backends and has to mean "an anime with episodes" too, and a reader that only
ever shows a list does not miss `status`. It is a statement about what
adopting *that path* would cost *this* app: the details screen loses its
status row, a detail fetch can no longer correct a title a search got wrong,
and the reader reports "no pages" for a site that blocked it.
...
Note what that costs and what it does not. `lib/source/` is derived from
**Mangayomi, which is Apache-2.0**, and that is why this app can be
permissively licensed today — and it stays that way right up until the bridge
is linked, at which point the *whole build* becomes GPLv3 regardless of how
little of the bridge it calls. Linking it for Aniyomi alone costs exactly as
much licence as linking it for everything.
...
### The order to do it in — slice 1 is cancelled

The plan was Mangayomi through the bridge first, because it is pure Dart and
proves the seam cheaply, then Aniyomi and Kotatsu. The two measurements above
kill the first half of that:

- it is not needed as a proof, because the bridge already resolves and
  analyses alongside `lib/source/`; and
- it is a **regression** in its own right — status, title/cover correction,
  and the difference between "no pages" and "blocked".

So `lib/source/` stays as the Mangayomi backend, and the bridge is worth
adopting for exactly one thing: **Aniyomi**, which is where the 1,396 packages
are. That is slice 2, and it needs the Android side
(`MethodChannel('aniyomiExtensionBridge')`, a runtime host APK). Kotatsu
(`plugin.jar`) is the same shape and can follow.

### What the Android side costs — measured, because it does not build as-is

`flutter build apk --debug` with the bridge added **fails**, three times over,
and every failure is a plugin pinned to a 2022-era toolchain. (This is also why
AnymeX itself still runs Gradle 8.11.1, where the first of them still works.)

| blocker | why | what it takes |
|---|---|---|
| `device_apps 2.2.0` | abandoned. `jcenter()`, removed in Gradle 9; AGP 4.0.1 on its own classpath; `compileSdkVersion 30` | replace. Used **only** by `AniyomiExtensions`, for `isAppInstalled` and `uninstallApp` — a two-method surface |
| `install_plugin 2.1.0` | abandoned. No `namespace`; `compileSdkVersion 28`; AGP 7.3.0 classpath; `legacy-support-v4` | replace. One method, `installApk(path, appId:)` |
| `file_picker` compiled against android-34 | this repo's own root-Gradle hook was being silently overwritten — see the mistakes table | `androidComponents.finalizeDsl`, fixed here |

With a stub standing in for each of the first two, **the debug APK builds**.
So the bridge is adoptable and the shims are small — both are Aniyomi-only, so
they are slice 2's work, and a replacement we write declares its own namespace,
which is why the Gradle fix here does not carry a namespace fallback.

What comes along whether this app calls it or not: `libtorrent_flutter`
(**12.3 MB of native torrent library per ABI**), `flutter_inappwebview
6.2.0-beta.2`, `jni`, `file_picker`, and QuickJS (`libqjs.so`, for the LnReader
and Sora engines). Measured on the debug APK: **198.9 MB → 239.9 MB**, of which
~33 MB is libtorrent across three ABIs. A manga reader pays that for a torrent
streamer it never opens.

### What this does not change

`lib/source/` stays, and with it every rule in "The source runtime" below —
`kBridgeLibraryUri`, extensions run unmodified, the ported files are
byte-identical upstream, the 55/55 ÷ 6/6 baseline. The deferred
`upstream-behaviour` issues still describe real upstream behaviour, and the
bridge ports the same files from the same place, so they describe its
behaviour too.

What the bridge adds is a **second backend**, not a replacement for the first.
`SourceMethods` here is the seam that has to grow a backend dimension — as
`Repo` has to grow `managerId` — and that is the shape the Kotlin repo
arrived at independently, after measuring the same trade and cancelling the
same removal.
...
- **Paged and continuous keep *separate* reading directions.** AnymeX stores
  ...
  which one is in force. **Every** site goes through it — both body builders,
  the page measurement, the reader's own control — rather than each reaching
  for the key it happens to want. That is structural rather than stylistic:
  `codeant-ai` caught the first version of this paragraph claiming it while the
  two builders still read `direction` and `webtoonDirection` directly, which is
  correct only for as long as each builder is used by exactly one layout.
...
- **Six actions, not AnymeX's eight, and the two it loses are why its editor
  ...
  assume two.
- **A geometry is edited through the *boundaries*, not the parts.** Three band
  sliders that must sum to 1 need the other two rewritten after every drag, and
  *which* one gives way is an invisible policy nobody chose. `n - 1` cut points
  have no such policy: the bands are the gaps between them, so the sum is
  algebraic rather than enforced. Measured over all 171 pairs the editor can
  reach on its 5% grid — **166 sum to exactly 1.0, worst error 1.1e-16**,
  thirteen orders inside `TapZoneProfile.tolerance`. So the editor structurally
  cannot produce a profile the writer refuses, and the refusal path exists for
  restores and imports alone. It still **reports**, and a refusal deliberately
  leaves the stored row alone: clearing it would turn one bad write into the
  loss of bands already authored, and the return value cannot say which
  happened.
- **A minimum band is what stops the editor handing back the failure the
  geometry removed.** Bands cannot gap or overlap, but a band of *zero* is
  unreachable — a dead zone, and indistinguishable from one deliberately set to
  do nothing. The cut sliders are bounded by their neighbours plus
  `kMinBandFraction`, so the invariant lives in the slider's range rather than
  in a check after the fact. A boundary pinned against both neighbours has
  nowhere to go, and `Slider` throws when `min == max` — so that row goes
  **inert**, visibly, through the same `enabled` path the switch uses. Widening
  the range instead is the fix that suggests itself and reintroduces the dead
  zone.
- **A reset forgets the row; it does not write the default into it.** The two
  are indistinguishable today and stop being so the moment the standard bands
  change: a user who never edited theirs should get the new default, and one
  who *reset* theirs asked for the default rather than for a copy of whatever it
  happened to be that day.
- **A preview draws the reader, or it is worse than no preview.** The band
  preview reads the same direction key the reader reads and mirrors when the
  reader would, so its leading edge is the one a tap will meet. Always drawing
  left-to-right would show the reverse of the app's own behaviour for most
  manga, with every file staying perfectly self-consistent — the `ReaderDefaults`
  shape, in the one screen whose entire job is to show what a tap does.
- **AnymeX's tap-zone screen applies the profile last *looked at*, not the one
  being *read in*.** Measured by grepping every writer: the only ones are its
  two segmented controls on that settings screen, plus a read-back at reader
  init. Nothing derives them from the live reading mode, so reading a webtoon
  after last opening its "Paged" tab makes the left third fire `prevPage` in a
  vertical reader. Here the selector chooses only which profile is being
  *edited*, the reader picks from its own `layout`, and a test asserts the
  editor writes **no** reader state at all.
...
- **The colour treatment is applied once, and the tint goes last.** AnymeX
  applies its filter through **two** mechanisms that are both unconditionally
  in the same `Stack`: `reader_view.dart:206` wraps the content in
  `ColorFiltered(ColorFilter.mode(colour, blendMode))`, and `:237` then stacks
  `ReaderContentOverlay`, whose painter draws the same colour in the same blend
  mode over the top. Read in the reference rather than inferred — and it is
  exactly what a survey misses, because the two live in different files and
  only one is named after colour. **What the doubling renders was not
  measured**: the obvious pixel probe deadlocks under `flutter test`, because
  `RenderRepaintBoundary.toImage` cannot complete inside `pumpAndSettle`. The
  structure is enough to decide it; the magnitude is not claimed. Order is the
  other half: desaturate or invert the *artwork*, then tint the result, because
  tinting first and desaturating after throws the tint away and is the one
  ordering that makes the control silently inert.
- **Greyscale and invert compose; AnymeX's are exclusive.** Its reader reads
  `if (greyscale) … else if (invert)`, so turning greyscale on leaves a live
  invert switch doing nothing — the defect this file forbids outright, and
  which this reader has now shipped three times. Composing is also strictly
  more capable: inverted greyscale is a real night-reading mode and is the one
  the exclusive version cannot reach.
- **It is a *dim*, not a brightness, and the name says so.** AnymeX's slider
  runs **−75 to 100** while its overlay reads only `brightness < 0` — measured
  by grepping every reader of the value — so **100 of its 175 units, 57% of the
  travel, change nothing**. Raising the screen past its system setting needs
  the platform's own brightness control and a plugin this app does not carry,
  so a control labelled Brightness here would be a fourth switch promising
  something that is not behind it. Stored as a positive magnitude, because a
  stored `-40` invites the next reader to wonder what `+40` does.
- **The dim is painted over the page and *under* the chrome.** AnymeX stacks
  its overlay above everything it draws. Dimming the controls along with the
  artwork makes the one surface a reader reaches for when the page is too
  bright the hardest thing on screen to read.
- **The dim's magnitude and its switch are separate keys.** "Zero means off"
  loses the setting every time it is toggled: a reader who dims to 40, turns it
  off and turns it back on wants 40, not 0.
...
| An entire subsystem was built while the reference app on disk already had it | `lib/source/` is 4,361 lines implementing the Mangayomi Dart runtime. `anymex_extension_runtime_bridge` — the plugin AnymeX depends on, whose repo is one `git clone` away and whose name is in AnymeX's own `pubspec.yaml` — implements the same thing from the same upstream, plus six other backends. Phase 1 never checked. This is the row two below it ("Building a screen without opening AnymeX's version of it") at the scale of the whole architecture, and the *third* time in this project: a screen, a widget, now a subsystem. The tell each time was identical — a survey (`ls`, `grep`, a pubspec skim) reported as a deep dive. A survey tells you what files exist; it does not tell you what they do. **Before building any layer, `clone` the reference's dependencies and read the seam, not the directory listing.** Cost: a stated rewrite rationale that had to be rewritten, and a catalogue ceiling 6x below what was available the whole time. Not the 4,361 lines, as it turned out — reading the bridge's `MangayomiSourceMethods` afterwards showed it drops publication status, cannot correct a title on a detail fetch, answers a blocked page list with an empty one, and rebuilds its interpreter on every call. So that subsystem stays and the bridge is adopted for Aniyomi instead. **The lesson survives the reprieve**: it is luck that the thing built blind turned out better, and the deep dive that would have found the bridge is the same one that would have found those six rows. |
| A text cap that hid the sentence, and a test that could not see it | `ChromeFeatureCard.description` and `ChromeTile.subtitle` were `maxLines: 2` + ellipsis. The suite's "a doubled system font does not clip or overflow" test passed anyway, because **an ellipsis overflow throws nothing** — it silently drops the end of the sentence, and `takeException()` is blind to exactly that. `RenderParagraph.didExceedMaxLines` is the question that was actually being asked. Found by `codeant-ai` on the description; the neighbour (`ChromeTile.subtitle`) was worse and unflagged, rendering the longest subtitle in the app. **Fifth** time a rule held in one file and not in its neighbour. The fix is not a threshold: both are prose in a scrolling list whose height is free, so the cap only ever buys density and pays in hidden words. Uncapped. The four caps that remain are layout invariants and stay — the header's two (its height is its own property), the tab label (the `TabBar`-overflow fix), and `ChromeTile.title` (a long title must ellipsise rather than shove its badge off screen, pinned by its own test). The general shape: before lifting a cap, ask whether it holds a layout invariant or merely a preference, because they look identical in the diff. |
...
| A careful decoder sitting behind a read that could not survive the row | `TapZoneProfile.decode` refuses every malformed shape and answers null so the reader falls back — and it was called as `TapZoneProfile.decode(key.get<String?>())`. `KvHelper.get` ends in `return val as T`, and its two re-widening guards match `T == double` and `T == int` only, so a row holding a *number* reached that cast as `String?` and threw before the decoder ran at all. Reproduced: `type 'int' is not a subtype of type 'String?' in type cast`. Found by `sourcery-ai`. The hazard is general — every `get<String?>` in the app shares it — and the narrow fix (read `Object?`, type-check at the call site) was taken deliberately over changing the primitive every feature reads, because that is a persistence change wearing a reader change's diff. The shape to remember: **defensive parsing is worth nothing if the call that feeds it can throw first**, so check what the *read* does before congratulating the decoder. |
| A label that reintroduced the thinking its own enum was collapsed to avoid | `ReaderAction.next` exists precisely because it means one unit of reading order *whatever the layout* — that is the argument for six actions rather than AnymeX's eight, written three lines above in the same file. It was then labelled "Next page", which is a page turn, which is the paged layout. Caught by `sourcery-ai`. Nothing failed: the label is correct in the layout most people read in, and wrong only in the one the collapse existed to serve. When a rename or a collapse is justified by a property, grep the strings too — the user meets the string, not the enum. |
| Four documents claiming thirds for bands that are 30/40/30 | The standard profile is AnymeX's own, whose middle band is deliberately the widest because it is the one a reader hits without aiming. "The standard thirds" went into `FEATURES.md`, `ROADMAP.md`, a doc comment and the PR body — and `FEATURES.md` is the file whose entire job is to not overstate. Caught by `codeant-ai`. A number is the cheapest thing in the world to check and the easiest to paraphrase into a lie. |
| A port carried a constant whose reason did not come with it | AnymeX's tap-zone preview is a 9/16 phone, which is right *there* because its editor is **nothing but** the preview. Dropped into a screen that also has boundary sliders, 9/16 across a 390px phone is a 636px box: header plus switch plus preview fills an 844px screen on its own, and every slider starts below the fold — so the reader would drag a boundary with the bands it moves off screen, which is the one thing that screen exists to show. `flutter analyze` clean, no overflow, no exception; it simply could not be used. The fix is a cap as a *share of the viewport* rather than a constant, so it holds on a tablet and in split screen too. The general shape: a number copied from a reference was correct for the composition it was in, and a port changes the composition — so carry the *reason* for a constant, not the constant. |
| A cap of zero erased the thing it was capping | `maxHeight: viewport * share` with `viewport == 0` does not shrink the preview, it removes it: the `AspectRatio` collapses, every band lands on the same point, and nothing throws. Found only because a test *produced* that state (below) and the failure read as a mirroring bug. Guarded now — a non-positive viewport falls back to uncapped — which is the same rule as `ChromeCard` dropping a zero-sigma `BackdropFilter` rather than scaling it: **at zero, remove the effect rather than scale it to zero**, because "too small to see" and "not there" render identically. |
| A widget test built `MediaQueryData()` instead of copying the real one | A fresh `MediaQueryData` carries **`size: Size.zero`**, so every `MediaQuery.sizeOf` under it reads zero — which collapsed the preview above, while the four width tests in the same file went on passing against a preview that was not on screen. They were measuring a harness, not a widget, and they were green. The failure surfaced in a *different* test (the mirror order), because all three bands reported the same centre and the sort silently became iteration order. `MediaQuery.of(context).copyWith(...)` through a `Builder` is the only correct spelling. **Eleventh** instance of a layout defect `flutter analyze` cannot see, and the second where a passing test was the thing hiding it. |
| A mutation anchor was rewrapped by `dart format`, for the fifth time | Two of six mutation patches in this slice failed to find their anchor — the height cap had become a three-line ternary and the selector's arrow a wrapped lambda. Both were caught by the helper's `assert`, which prints "patch NOT applied, any result is meaningless" and exits non-zero rather than reporting a silent no-op as a pass. That helper exists *because* an earlier `sed` in this repo matched nothing and its green run was read as a hollow guard. It is now the fifth occurrence, so the lesson is no longer "remember to check": **use a tool that fails loudly on a missed anchor**, and read the failure count rather than the failure list. |
| An invariant asserted by five documents and "verified" by a test that iterated a set the code could not produce | `Slider.divisions` divides **that slider's own `max - min`**, not the axis — and these bounds are dynamic, because each boundary is fenced in by its neighbours. So a fixed `divisions: 20` gave the standard profile a **3%** step, and moving the second boundary to 0.5 changed the first slider's step to **2%**: not merely off the documented 5% grid but *not constant*, drifting under the user as they worked. Meanwhile a doc comment, `CLAUDE.md`, `ROADMAP.md`, `FEATURES.md` and the PR body all called it a 5% grid, and a 171-pair test "proved" the sum-to-one property **over `i/20, j/20`** — an imaginary grid the editor never visited. The measurement was real and the code did not implement it. Found by `codeant-ai`, Major, correct, and worse in fact than in the report. Two rules meet here and the second is new. The old one: **a number is the cheapest thing to check and the easiest to paraphrase into a lie** — this is the 30/40/30 row again, in the same session, by the same hand. The new one: **when a test enumerates the inputs a feature can take, it must derive them from the feature, not restate the intention** — restating it makes the test agree with the prose and neither with the code, and that is worse than no test, because it reads as verification. The fix makes the grid real (divisions from the range, plus a snap in `onChanged` so an imported off-grid profile is pulled onto the grid rather than keeping its offset) and adds the assertion nothing had: the **step itself**, at the opening state *and* after the other boundary moves, because the bug changed the step as a side effect of an unrelated drag. |
| The same test, wrong a second time, one layer below where it was just fixed | Having been caught enumerating `i / 20` for sliders that stepped in 3%, the loop was rewritten to derive its grid from `kBandStep` — and **the numbers changed**. `i * 0.05` is not `i / 20`: measured, `6 * 0.05` is `0.30000000000000004` and `19 * 0.05` is `0.9500000000000001`, so the count of pairs summing to *exactly* 1.0 fell from **169 to 166**. The production snap is `(v / kBandStep).round() * kBandStep`, the multiply form — so 166 was always the true figure, and 169 described a set the editor has never once produced. That wrong number was in a doc comment, a second doc comment, `CLAUDE.md`, `ROADMAP.md`, `FEATURES.md` and the PR body. The smallest band the editor can emit is likewise `1 - 19 * 0.05` = `0.04999999999999993`, under `kMinBandFraction` by 7e-17 — harmless, but only because the assertion carries float slack rather than comparing exactly. Found by rewriting the test to derive from the feature and watching it fail, which is the whole argument for deriving. **Two rules, and the second is the sharper one.** First: when a test enumerates what a feature can produce, it must use the feature's *own arithmetic*, not an algebraically equal rewrite — `i / steps` and `i * step` are equal in mathematics and different in doubles, and the one that matches the code is the only one that measures it. Second: **fixing a measurement is not the same as re-taking it.** The step-size fix made the grid real and the 169 was carried forward untouched, because it had been "measured once"; it had been measured against the wrong arithmetic both times. A number re-stated after the code changes under it is a fresh claim and needs a fresh measurement. |
| A restore that silently did nothing, because `git checkout` cannot restore an untracked file | Two mutations were run back to back on the same file. The helper's `shutil.copy(path, path + '.bak')` **overwrote the first mutation's backup with the second's mutated text**, so `mv $L.bak $L` restored a half-mutated file; the `git checkout -- $L` behind it was a no-op, because the file was new and untracked. The result was `reader_display_layer.dart` applying its tint **twice** — AnymeX's exact bug, reintroduced by the tooling written to catch bugs — and the tell was a *baseline* run that came back `+59 -2` instead of green. Two fixes, and the second is the durable one: the file was repaired by hand, and the helper now **refuses** when a `.bak` already exists rather than clobbering it. The general shape: `git checkout` is this table's existing destroyed-work row wearing the opposite failure — there it deleted work it should not have, here it restored nothing while reporting success, and both come from reaching for it as an undo it is not. **Always re-run the baseline after a mutation series and check it is green**, because a failed restore and a real regression are the same output. |
| A number in the parity checklist that came from the wrong app | `FEATURES.md` listed "reader background (9 options)". AnymeX has **four** (`reader_view.dart:170-175`: white, `#303030`, the scaffold colour, black); nine is Komikku's count, and it reached the file that exists to not overstate without anyone opening the reference it was filed under. Same shape as the 30/40/30 row and the 169 row, and the third time in two days — the recurring tell is a number written next to a reference's name without the reference being read. |
...
| A framework migration whose whole content was a line that had to be *deleted* | `onReorder` reports a reorder's destination as an index into the list **before** the dragged item is removed, so every implementation carries `if (oldIndex < newIndex) newIndex -= 1;`. `onReorderItem`, which deprecates it, applies that correction itself. So the migration is: swap the callback name **and remove the line you already wrote**. Keeping it — the obvious thing, since it was sitting right there and looks like defensive arithmetic — lands every downward drag one row short, with no error, no exception and nothing else to fail. Found by writing a drag test for it; the mutation that restores the line fails that test (`+26 -1`). The general shape: when a deprecation says the replacement "handles X for you", the diff is not additive, and the thing to grep for is the code that used to handle X. |
| A reload helper called from a path that had just changed state | The category sheet's `_create` set the new category ticked and then called `_load()` to refresh the list. `_load()` re-seeds **both** `_checked` and `_initial` from the row — `_initial` being what Save compares against to decide whether anything changed. So a freshly made category came back **unticked**, *and* Save then concluded nothing had changed and wrote nothing at all. Neither half is visible in the diff: `_load()` is named for the thing it does and does one more. The tell is general — a method that seeds "current" *and* "original" state is safe at init and wrong everywhere else, so check what a refresh helper resets before calling it from an edit path. |
| A survey of declared-but-unread keys, whose own first run was mis-parsed | `FEATURES.md` gained a test counting honoured `ReaderKeys` because a prose tally had been wrong three times. Pointing the same count at **every** key enum found **46 dead of 86**: `ReaderKeys` 21/39, `DynamicKeys` 6/8, `LibraryKeys` 5/8, `General`/`DownloadKeys`/`UpdateKeys` 3 each, `ServiceKeys` 3/3, `ThemeKeys` 2/11, `SourceKeys` none. Every one is the defect this file states outright — a control that writes a key nothing reads — waiting to be built on top of. **The first run of that count was itself wrong**, and the correction is the more useful half. It reported "47 of 83" and claimed `ServiceKeys` was **4 of 4 dead, duplicating `UpdateKeys` member for member**. It is neither: `ServiceKeys` is `{serviceType, homePageCards, homePageCardsMal}`, AnymeX's multi-service idea that this app does not have. The shell loop ranged `sed -n '/^enum X {/,/^}/p'`, and `ServiceKeys` is declared **on one line** — so the range opened at it and ran on to the *next* closing brace, swallowing `UpdateKeys`' members and attributing them to its neighbour. The claim reached this table and a commit message before anything re-measured it. Two lessons, and the second is the sharper: a parser that assumes one formatting of the thing it parses is wrong about exactly the entries formatted the other way, silently and plausibly; and **the audit written to catch restated numbers restated one of its own** — so a count is not evidence until the thing that produced it has been read as carefully as the code it measures. |
