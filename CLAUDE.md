...
| A survey of declared-but-unread keys, run once and then not repeated | `FEATURES.md` gained a test that counts honoured `ReaderKeys` because a prose tally had been wrong three times. Running the same count across **every** key enum found **47 dead of 83**: `ReaderKeys` 21/39, `DynamicKeys` 6/8, `LibraryKeys` 5/8, `General` and `DownloadKeys` and `UpdateKeys` 3 each, `ThemeKeys` 2/11 — and `ServiceKeys` **4 of 4**, an enum that duplicates `UpdateKeys` member for member and is read by nothing at all. Every one of those is the defect this file states outright (a control that writes a key nothing reads) waiting to be built on top of. The lesson is not the number: it is that the guard written for one enum was never pointed at its neighbours, which is this table's most-repeated shape in its cheapest possible form — one `grep` loop, run for the first time months later. |
