...
| A refused Sourcery review still spends the weekly budget | Sourcery has **two** limits: 150,000 diff characters per PR, and **250,000 per 7 days across the account**. #1 and #27 were each refused for the first — and still consumed the second, buying no review at all. The budget ran out on #32, where the check reads **`skipped`**, not failed, which is easy to read as a pass. Guides are still generated when the budget is gone; only the findings pass stops. So an oversized PR is worse than useless: it costs the reviews that later PRs would have had. |
