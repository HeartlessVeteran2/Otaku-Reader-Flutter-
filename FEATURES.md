...
**Settings** — [ ] **settings search with deep-link and row highlight** · [~] theme — seed, Material You, custom, every variant and OLED all ship; no schedule · [x] **per-cover auto theme** · [ ] font family picker · [ ] **reorderable + hideable nav tabs** · [ ] UI multipliers (glow/radius/blur/roundness/animation duration) · [ ] card/carousel/navbar style registries · [ ] grain texture · [ ] liquid wallpaper · [ ] refresh-rate picker · [ ] advanced (user-agent override, DoH, verbose logging, clear cookies, battery optimisation, don't-kill-my-app) · [ ] storage manager with per-bucket sizes + auto-clear threshold · [ ] logs to file
